...
- [x] 2.5 Enforce the v1 permission matrix: Supervisor — gas-reading monitor + reading entry; Ops Manager — work orders, FCC lifecycle, certify, intake review; Admin — all Ops plus stock adjust; Client — own FCCs in `/portal` only; Executive — authenticated role exists, but no dashboard (SDD open item; do not invent one).
...
- [x] 3.2 Implement `GET` / `PATCH` `app/api/work-orders/[id]/route.ts`.
- [x] 3.3 Auto-generate `WO-YYYY-#####` sequentially per year (do not derive the number from `workOrders.length`). Accept an externally supplied unique code; set `source` to `client_supplied` | `auto_generated` | `website`.
...
- [x] 3.5 Wire the dashboard active-work-order table to `GET /api/work-orders` and add search/filter controls. Keep existing KPI cards, but compute them from the API (including “certified this period” as a real date window, not “all certified”).
- [x] 3.6 Restrict the new-work-order wizard step 0 to work-order identity only. Do **not** insert `gas_readings` rows on create (that is 6.1, keyed off Date Fumigant Placed). Creating a WO should yield `fccs.status = draft` (or `in_progress` only once fumigation description is saved — match SDD lifecycle; do not jump to in_progress solely because the wizard finished).
...
- [x] 4.2 Implement `POST /api/stock/adjust` (Admin only): wrap `stock_movements` insert (`addition` | `adjustment` | `deduction`) + `stock_levels.quantityOnHand` update in a single DB transaction. Require a note. Never allow negative on-hand.
- [x] 4.3 Rewrite the inventory page off `adjustStock()` ±10/±50 shortcuts: Admin-only, custom quantity, note field, and a visible movement history. Supervisors/Ops may view levels; only Admin mutates.
- [x] 4.4 Drive formulation dropdowns from live stock (reuse `availableFormulations` rules against DB rows, including `cropType = both`).
- [x] 4.5 Surface low/zero-stock on the dashboard stock-alerts card from API data. Enqueue Admin notification on crossing threshold (wired in 9.6).
...
- [x] 5.2 Implement `PATCH /api/fccs/[id]/si`: client (own FCC) and Ops/Admin may edit until `shipping_instructions.lockedAt` is set; return 403/409 if locked. Include every SDD SI field plus **Fumigation Contractor, pre-filled** to Primon Enterprises Limited (not currently on `ShippingInstructions` in `lib/types.ts`).
- [x] 5.3 Scope `/portal` to the authenticated client’s work orders by `clientId`. Remove the hardcoded `client.includes("Alliance One")` filter. Clients see the full FCC read-only except SI pre-certification.
- [x] 5.4 Implement `POST`/`PATCH /api/fccs/[id]/fumigation-description`. Filter formulation by fumigant + crop type and current stock. On first save, in one transaction: write the description, insert `stock_movements` type `deduction`, decrement `quantityOnHand`. Reject if stock is insufficient. Set FCC `in_progress` when description is complete.
- [x] 5.5 Allow SI to remain incomplete when description is saved (out-of-order completion). SI completeness must not block fumigation or readings.
- [x] 5.6 Wire wizard steps 1–2 and the certificate SI editor to these APIs. Until 5.4 ships, do not show copy that says stock will be deducted; after 5.4, keep that copy because it will be true.
- [x] 5.7 Show Relative Humidity on the certificate 6-day table (logged in the monitor form today, omitted from `certificate-view.tsx`).
...
- [x] 6.1 Recording **Date Fumigant Placed** (not work-order create) inserts six `gas_readings` rows (`dayNumber` 1–6) with sequential `readingDate`s. Until Ops confirms the SDD open item on weekends/holidays, use consecutive calendar days and record that assumption in the capsule log.
- [x] 6.2 Implement `POST /api/readings`: require numeric Airspace and Probe/Case; temps and RH optional. Derive and **store** `status` (`compliant` if both ≥ 600ppm, `critical` if either < 600). Do not recompute status only on read. Persist `enteredBy` / `enteredAt` from the session.
- [x] 6.3 If status is `critical`: set FCC `flagged`, enqueue Ops Manager/Admin notification (9.3), and keep the day red until a corrective action is logged. Treat `flagged` as the SDD schema enum value; preserve the previous lifecycle status in `audit_log.diff` so certification still follows `draft → in_progress → under_review → certified`.
- [x] 6.4 Implement `POST /api/readings/[id]/corrective-action`: free-text description, timestamp, `loggedBy` from the session (**never hardcode** “Grace Phiri”). Set day status to `action_taken`. When no days remain `critical`, move FCC to `under_review` (six resolved days) or back to `in_progress` if readings are still pending.
- [x] 6.5 Add a fumigation close-out form for Aeration Began, Aeration Completed, and Duration / Total Hours Under Gas on `fumigation_closeout`. Do **not** auto-stamp these in `certify()`.
- [x] 6.6 Wire `/dashboard/monitor` and `/dashboard/monitor/[id]` to the APIs. Keep the existing gauges and sequential “next pending day” UX unless Ops asks to log days out of order.
- [x] 6.7 Block certification (7.2) until all six days are `compliant` or `action_taken` **and** close-out is recorded. Write `audit_log` for every reading and corrective action.
...
- [x] 7.1 Add the certification UI for the three SDD roles: Supervising Fumigator, For the Supplier, Certifying Officer. Persist `signatures` (`signerName`, `signedAt`, role). System timestamp stands in for wet signature.
- [x] 7.2 Implement `POST /api/fccs/[id]/certify` as one DB transaction: require three signatures, six resolved readings, and close-out; insert/confirm signatures; set `fccs.status = certified`, `certifiedAt`, `certifiedBy`; set `shipping_instructions.lockedAt`; assign sequential `FCC-YYYY-######` (not `Math.random()`). Follow-up async job for QR/PDF is allowed after the commit (SDD §A.4).
- [x] 7.3 Generate a QR whose payload is the public verify URL. Until `verify.primon.mw` exists, that is `{NEXT_PUBLIC_APP_URL}/verify/{FCC-YYYY-######}`; set `NEXT_PUBLIC_VERIFY_ORIGIN` later to switch to `/fcc/{number}`. `qrCodeUrl` / `verificationUrl` store that URL (QR is rendered at print time; no Blob image required).
- [x] 7.4 Certified FCC PDF is generated client-side from the locked sheet (html2canvas/jsPDF) and offered only when `status = certified`. Database remains the source of truth; Blob PDF deferred until a token is configured.
- [x] 7.5 Implement public `GET /api/verify/[certificateId]` (rate-limited, payload-validated, read-only, non-sensitive summary). Public page at `/verify/[certificateId]` (and `/fcc/[certificateId]`). Do not use `/certificate/[workOrderId]` as the public verifier.
- [x] 7.6 Gate portal and certificate-page PDF buttons on `certified`. Target: certified FCC downloadable within 5 seconds of the certify action (SDD success metric).
- [x] 7.7 Enqueue client + Ops Manager certified-FCC notifications (in-app now; email in 9.4). Write `audit_log` for certification.
...
2026-09-23 — Deployed role-based access control (RBAC). `middleware.ts` now prevents clients from accessing `/dashboard` and staff from accessing `/portal`, while trapping `supervisor` roles exclusively in `/dashboard/monitor`. `components/sidebar.tsx` updated to use `useCurrentUser` and conditionally render links via an `allowedRoles` array. `package.json` build script patched to include `prisma generate && prisma migrate deploy` for Vercel CI/CD. `README.md` rewritten to reflect the Next.js/Prisma/Neon stack and first-admin onboarding. Tasks 2.5 and 2.6 complete. Auth phase (2.0) is officially closed.
2026-09-23 — Completed Phase 3.0 Work Order Management. Implemented `GET`/`POST /api/work-orders` with sequential annual code generation (`WO-YYYY-#####`), client-scoped RBAC filtering, optional header metadata (Sales Order No., Shipment No., Delivery No.), and `scale` ("industrial", "smallholder", "household"). Created `GET`/`PATCH /api/work-orders/[id]` and `GET /api/stock`. Connected `/dashboard` active work orders table with search and filters, and updated `/dashboard/work-orders/new` wizard to submit work order creation via database API. All Task 3.0 sub-tasks (3.1–3.6) are complete.
2026-09-24 — Completed Phase 4.0 Inventory, Formulations & Stock Movements. Updated `GET /api/stock` to return stock levels and movement log. Created `POST /api/stock/adjust` with Admin-only access, atomic stock level updates, movement logging, audit trail (`audit_log`), and non-negative stock validation. Rewrote `/dashboard/inventory` with RBAC enforcement (Admin mutation vs Staff view-only), stock adjustment modal, shortcuts (+10, +50, +100, -10, -50), mandatory audit notes, and append-only movement history table. Updated formulation wizard dropdowns to fetch live database stock levels. All Task 4.0 sub-tasks (4.1–4.6) are complete.
2026-09-24 — Completed Phase 5.0 FCC Draft, Shipping Instructions & Fumigation Description. Implemented `GET /api/fccs/[id]`, `PATCH /api/fccs/[id]/si` (with post-certification lock check, pre-filled 'Primon Enterprises Limited', and audit log diffs), and `POST`/`PATCH /api/fccs/[id]/fumigation-description` (with transactional stock deduction, insufficient stock rejection, status progression `draft → in_progress`, and audit logging). Scoped `/portal` to authenticated client's work orders by `clientId`. Updated wizard steps 1–2 to submit to live APIs. Added RH % column to certificate 6-day table. All Task 5.0 sub-tasks (5.1–5.8) are complete.
2026-09-25 — Fixed the post-5.0 Neon driver regression. Prisma 6.7 `PrismaNeon` is a factory that takes `{ connectionString }` and builds the WebSocket Pool internally; passing an existing `Pool` made new clients fall back to localhost / OS user (missing DATABASE_URL) and broke Better Auth. Fail-fast lazy client, moved `@prisma/adapter-neon` + `@neondatabase/serverless` to runtime dependencies, marked them (and `ws`) as Next server externals. Verified login, transactional stock adjust, and fumigation-description deduction.
2026-09-25 — Completed Phase 6.0 Gas-Reading Monitor, Flagging & Close-Out. `POST /api/fccs/[id]/date-placed` inserts six pending `gas_readings` on consecutive calendar days (weekend/holiday skip still an SDD open item). `POST /api/readings` stores 600ppm status; critical sets FCC `flagged` and enqueues in-app Ops/Admin notifications. `POST /api/readings/[id]/corrective-action` uses session actor (not a hardcoded name) and returns FCC to `in_progress` or `under_review`. Close-out aeration fields are explicit (`PATCH /api/fccs/[id]/closeout`); Certify stays disabled until Phase 7. Monitor list/detail read live APIs. All Task 6.0 sub-tasks (6.1–6.7) are complete.
2026-09-25 — Completed Phase 7.0 Certification, QR & public verify. `POST /api/fccs/[id]/certify` requires three named signers, six resolved readings, and aeration close-out; assigns sequential `FCC-YYYY-######`, locks SI, and stores the verify URL. Public summary card at `/verify/[certificateId]` (alias `/fcc/...`); QR encodes that URL on the current host until `NEXT_PUBLIC_VERIFY_ORIGIN` is set. PDF download gated on `certified`. In-app certified notifications queued (email in 9.4). All Task 7.0 sub-tasks (7.1–7.7) are complete.
